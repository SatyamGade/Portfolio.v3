...
const AddProject = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      axios.post('https://portfolio_backend.satyagade8055.workers.dev/api/auth/admin/project', {
        title: info.title,
    ...
    <div className={`py-2 px-4 md:py-4 md:px-8 lg:px-28 w-full h-screen flex justify-center items-center ${darkmode ? "text-Txt bg-bodyBg" : "text-black bg-white"}`}>
      <form className={`w-fit p-6 lg:py-8 lg:px-10 rounded ${darkmode ? "bg-navbarBg text-Txt" : "border"}`} onSubmit={handleSubmit}>
        <div className="text-center">
          <h1 className="text-2xl lg:text-3xl">Add New Project</h1>
        </div>
        <div className="mt-8">
        ...
        <div>
          <input required className={`border-b w-full border-solid outline-none transition-all ease-in-out mt-3 ${darkmode ? "border-borderClr bg-navbarBg text-Txt caret-Txt focus:border-Txt" : "focus:border-black border-gray-400"}`} type="text" placeholder="Description" name="description" value={info.description} onChange={handleChange} />
        </div>
        <div>
        ...
        <div>
          <input required className={`border-b w-full border-solid outline-none transition-all ease-in-out mt-3 ${darkmode ? "border-borderClr bg-navbarBg text-Txt caret-Txt focus:border-Txt" : "focus:border-black border-gray-400"}`} type="text" placeholder="Source Link" name="sourceLink" value={info.sourceLink} onChange={handleChange} />
        </div>
        <div>
          <input required className={`border-b w-full border-solid outline-none transition-all ease-in-out mt-3 ${darkmode ? "border-borderClr bg-navbarBg text-Txt caret-Txt focus:border-Txt" : "focus:border-black border-gray-400"}`} type="text" placeholder="Deployed Link" name="deployedLink" value={info.deployedLink} onChange={handleChange} />
        </div>
        <div>
          <textarea required className={`border-b w-full border-solid outline-none transition-all ease-in-out mt-3 ${darkmode ? "border-borderClr bg-navbarBg text-Txt caret-Txt focus:border-Txt" : "focus:border-black border-gray-400"}`} placeholder="Technology Used" name="tech" value={tech} onChange={(e) => {
